...
if ($search !== '') {
    ...
    $search_sql = " AND (student_no LIKE '$s' OR last_name LIKE '$s' OR first_name LIKE '$s' OR email LIKE '$s')";
}
$sql = "SELECT * FROM tblstudent WHERE deleted_at IS NULL $search_sql ORDER BY student_id DESC";
$res = $conn->query($sql);
...
if ($res && $res->num_rows > 0) {
    while ($r = $res->fetch_assoc()) {
        ...
        $pdf->Cell(40,7,$student_no,1,0); // Student No wider
        ...
        $pdf->Cell(30,7,$birth,1,1);      // Birthdate
    }
} else {
